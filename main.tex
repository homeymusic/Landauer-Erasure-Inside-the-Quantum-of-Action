...
\text{Bit} \propto \text{It}
\end{equation}

\begin{equation}
\label{eq:MainIdeaText}
\text{Thermodynamic Erasure} \le \text{Kinematic Encoding}
\end{equation}
...
\caption{\textbf{The quantum of action} (schematic, 1D case). By geometrically translating the statistical Heisenberg limit, $\sigma_q \sigma_p \ge \hbar/2$, the classical semi-axes $\Delta q = \sqrt{2}\sigma_q$ and $\Delta p = \sqrt{2}\sigma_p$ establish the relation $\pi \Delta q \Delta p \ge h/2$. In phase space, this is the classical action area, $A \ge h/2$. Confined by these classical boundaries, the interior resolves into a conjugate pair of squeezed quantum blobs, $a_q = h/2$ and $a_p = h/2$, whose sum is the quantum of action, $h$. Specifically, $a_q$ is squeezed by the reciprocal boundary $\delta q = \hbar / \Delta p$ to area $\pi \delta q \Delta p = h/2$, while $a_p$ is squeezed by $\delta p = \hbar / \Delta q$ to area $\pi \Delta q \delta p = h/2$. These orthogonal blobs enclose their interior ellipse with quantum action area, $a \le h/2$, evaluated as $\pi \delta q \delta p = \pi \hbar^2/(\Delta q \Delta p)$. This inverse scaling geometrically enforces the correspondence principle: as the classical action area $A \ge h/2$ increases, the squeezed blobs $a_q=h/2$ and $a_p=h/2$ compress into fine cross hairs, shrinking the enclosed quantum action area $a \le h/2$ (see Fig.~\ref{fig:encoding_grid}). This geometric hierarchy establishes quantum and classical relationships $a \le h/2 \le A$ and $a = (\pi \hbar)^2 / A$.}
...
and their sum is the quantum of action$h=a_q+a_p=a_h$ with areas $a_q=\pi \delta q \Delta p = h/2$ and $a_p\pi \Delta q \delta p = h/2$:
...
Furthermore, Zurek showed that quantum interference of the compass state creates structures bounded by these squeezed boundaries, with rectangular area $a_z = \delta q \delta p$ \cite{Zurek2001}. Generalizing to symplectic geometry, the unique maximum-volume shape inscribed within this convex body is the Fritz John ellipsoid \cite{John1948}, shown as the interior uncertainty ellipse in Fig.~\ref{fig:quantum_of_action}. While the quantum blobs cannot be compressed smaller than de Gosson's $h/2$ limit \cite{DeGosson2022}, squeezing the blobs increases their resolving power forcing this inner geometry to be strictly smaller:
...
\begin{figure}[htbp!]
    \centering
    \includegraphics[width=\linewidth]{figures/encoding_grid.png}
\caption{\textbf{Position densities for the harmonic oscillator}.}
\label{fig:encoding_grid}
\end{figure}
...
Where the erasure paths from the least-action microstate $(q^\natural, p^\natural)$ to the blob centers $(q_b, p_b)$ are bounded by $|\epsilon_q| \le \delta q$ and $|\epsilon_p| \le \delta p$.

\subsection{Anatomy of the Erasure Action}

\begin{figure}[t!]
    \centering
    \includegraphics[width=\linewidth]{figures/encoding_path.png}
\caption{\textbf{Kinematic encoding and thermodynamic erasure} (schematic).}
\label{fig:encoding_path}
\end{figure}
...
\section{Landauer Density}

Standard phase-space formulations of quantum mechanics traditionally employ the Wigner quasi-probability distribution $W(q,p)$ \cite{Wigner1932}. In the context of information erasure, however, we instead introduce the Landauer erasure density $L(q,p)$ as a sum of discrete Iverson brackets \cite{Knuth1992}—mathematical switches that snap to exactly 1 when a coordinate is occupied and 0 otherwise—over the quantum blobs:

\begin{equation}
\label{eq:JointErasure}
L(q,p) = \sum_{b} \left[ q = \frac{A}{\pi \hbar} \epsilon_{q,b} \right] \left[ p = \frac{A}{\pi \hbar} \epsilon_{p,b} \right]
\end{equation}
...
For any phase-space coordinate, $x \in \{q, p\}$, an encoding function selects the least action microstate $x^\natural$ from among infite possible microstates $\{x_\mu\}$ inside the squeezed boundary $\delta x$ centered on the blob coordinate $x_b$, yielding a binary sequence $s_x$:

\begin{equation}
\label{eq:EncoderFunction}
(x^\natural, s_x) = \operatorname{ENCODE}(x_b, \delta x)
\end{equation}
...
Physical erasure of this sequence uncomputes the microstate back to the ground state ($x_0$). This process expends Landauer action ($\Delta A_L = N_x E_L \tau$) to the environment, yielding the phase-space displacement $\epsilon_x$:

\begin{equation}
\label{eq:ErasureFunction}
\epsilon_x = \operatorname{ERASE}(s_x, x^\natural, x_b), \quad \text{expends} \quad \Delta A_L = N_x E_L \tau
\end{equation}

The vector $\mathbf{\epsilon} = (\epsilon_q,\epsilon_p)$ is the distance from the least-action state $(q^\natural, p^\natural)$ to the blob center $(q_b, p_b)$. The observed classical coordinates $(q,p)$ are these displacements scaled by the geometric capacity $\frac{A}{\pi \hbar}$:
...
The physical erasure of the selected microstate unfolds over time. As the system clears these coordinate bits, the microstate steps closer to the ground state $A_0$. Because the classical coordinates scale directly with these squeezed offsets, they track this stepwise erasure process:
...
When the bits are fully erased ($\epsilon_q = \epsilon_p = 0$), the coordinates rest at the groud state $(q_0, p_0)$. We observe this mechanical loss of information as quantum decoherence.
...
\begin{algorithmic}[1]
\Function{ENCODE}{$\widetilde{x_b}, \widetilde{\delta x}$}
    \State $\text{left} \gets (-1, 0)$
    \State $\text{num} \gets 0, \quad \text{den} \gets 1$
    \State $\widetilde{x^\natural} \gets \text{num} / \text{den}$
    \State $\text{right} \gets (1, 0)$
    \State $s_x \gets \text{``''}$
    \While{$|\widetilde{x^\natural} - \widetilde{x_b}| > \widetilde{\delta x}$}
        \If{$\tilde{x}^\natural < \tilde{x}_b$} 
...
\begin{algorithmic}[1]
\Function{ERASE}{$(\bar{s}_x)^R, \widetilde{x^\natural}, \widetilde{x_b}$}
    ...
    \While{$N_x > 0$}
        ...
        \State $N_x \gets |(\bar{s}_x)^R|$
    \EndWhile
    \State $\widetilde{\epsilon_x} \gets \widetilde{x^\natural} - \widetilde{x_b}$ \Comment{The normalized phase-space displacement}
...
\caption{\textbf{Kinematic encoding and thermodynamic erasure tree} (schematic).}
