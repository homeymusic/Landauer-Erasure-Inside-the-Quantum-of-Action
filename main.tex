...
\begin{abstract}
Quantum microstates are operationally indistinguishable inside Planck's quantum of action. We show that Landauer's principle compels the thermodynamic erasure of the minimal-action microstate, yielding the observed classical state. This erasure granulates the continuum, selecting the discrete configurations of quantum mechanics at low action and smooth classical trajectories at high action.
\end{abstract}
...
Let phase-space coordinates be $(q,p)$, where $q$ is position and $p$ is momentum. According to the Heisenberg uncertainty principle \cite{Heisenberg1927}:
...
This demonstrates that the Margolus-Levitin limit for state evolution corresponds exactly to the maximal erasure time, naturally setting the physical timescale for quantum decoherence.
...
Planck demonstrated \cite{Planck1906} that a classical phase space of area $A$ has a capacity of $A/h$. Expanding the quantum of action $h$ into its pair of quantum blobs (Eq.~\ref{eq:Blobs}), we scale this macroscopic capacity to construct the boundaries of erasure action:
...
The area of the conjugate blob pair is strictly the quantum of action, $h$. Because this fundamental space can only ever fund the minimal bit budget ($K_q = 1, K_p = 1$), the action required to support a macroscopic bit budget ($K \gg 1$) must be provisioned by the classical scale of the system. The multiplier $A/h$ provides this capacity, scaling the quantum of action $h$ up to the macroscopic action necessary to support the total bits in $K$.

\subsubsection{The Geometric Squeezing ($h/2$)} 
When a macroscopic classical boundary expands (e.g., increasing momentum $\Delta p$), symplectic geometry strictly conserves the area of the underlying de Gosson blob at exactly $h/2$. Because this area cannot grow, the blob must physically squeeze Zurek's conjugate quantum uncertainty ($\delta q$) to compensate. 

\subsubsection{The Resolving Power ($K$)} 
This sub-Planck squeezing acts as the resolving power of the phase space. To pinpoint a microstate within increasingly fine spatial crosshairs ($\delta q \to 0$), the universe must expend more algorithmic information ($K_q \to \infty$). Therefore, macroscopic boundaries enforce microscopic resolution, which in turn demands a larger bit budget:
