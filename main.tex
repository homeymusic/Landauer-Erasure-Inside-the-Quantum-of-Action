...
Information erasure is fundamentally bounded by the classical action space:
...
Saturating the Heisenberg bound ($\sigma_q \sigma_p = \hbar/2$) yields the fundamental minimum area:
...
Expressing Zurek's reciprocal limits as phase-space areas directly yields a conjugate pair of de Gosson's invariant capacities (see Fig.~\ref{fig:erase_schematic}):
...
Transitioning from a single quantum blob to a pair of quantum blobs with strict reciprocal proportionality ($\delta q \propto 1/\Delta p$ and $\delta p \propto 1/\Delta q$) evokes the \textit{r\'{i}astrad} of the Old Irish hero C\'{u} Chulainn:
...
Because the simultaneous existence of both limits is required to bound the space, the total quantum of action $h$ is defined by their geometric sum:
...
Applying this dual-capacity definition of $h$ to the erasure limit (Eq.~\ref{eq:Hero}) partitions the thermodynamic cost into its orthogonal coordinate and momentum components:
