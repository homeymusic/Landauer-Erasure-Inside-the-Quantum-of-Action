...
\subsection{Erasure Action in Phase Space}

\begin{equation}
\label{eq:SimpleLanguagePhaseSpaceHero}
\text{Erasure Action} \le (\text{Quantum Capacity} \times \text{Pair of Blobs})
\end{equation}

\begin{equation}
\label{eq:HeroPhaseSpace}
...
$$
\texttt{erase}(q_\mu, \delta q) \rightarrow (q^*, s_q^*)
\quad \text{and} \quad
\texttt{erase}(p_\mu, \delta p) \rightarrow (p^*, s_p^*)
$$
...
Each microstate erases into the minimal-action state $(q_\mu, p_\mu) \rightarrow (q^*, p^*)$: $\epsilon_q = q_\mu - q^*$ and $\epsilon_p = p_\mu - p^*$, such that $|\epsilon_q| \le \delta q \le \Delta q$ and $|\epsilon_p| \le \delta p \le \Delta p$. The observed macroscopic state $(q,p)$ is the phase-space displacement $(\epsilon_q, \epsilon_p)$ scaled by the quantum capacity $A/h$:
...
\subsection{Dimensionless Erasure Action and the Hero Bound}

Starting from the exact geometric erasure action derived from the cross-conjugate boundaries:
\begin{equation}
\label{eq:ExactGeometricAction}
\text{Erasure Action} = \frac{A}{h} (\pi |\epsilon_q| \Delta p + \pi \Delta q |\epsilon_p|)
\end{equation}

We recall the areas of Zurek's conjugate quantum blobs (Eq.~\ref{eq:BlobAreas}):
\begin{equation}
\pi \delta q \Delta p = \frac{h}{2} \implies \pi \Delta p = \frac{h}{2 \delta q} 
\quad \text{and} \quad 
\pi \Delta q \delta p = \frac{h}{2} \implies \pi \Delta q = \frac{h}{2 \delta p}
\end{equation}

Substituting these macroscopic bounds into Eq.~\ref{eq:ExactGeometricAction} cleanly factors out Planck's constant:
\begin{align}
\text{Erasure Action} &= \frac{A}{h} \left( |\epsilon_q| \frac{h}{2\delta q} + |\epsilon_p| \frac{h}{2\delta p} \right) \nonumber \\
&= \frac{A}{2} \left( \frac{|\epsilon_q|}{\delta q} + \frac{|\epsilon_p|}{\delta p} \right)
\label{eq:SimplifiedErasureAction}
\end{align}

By defining the dimensionless relative quantization errors $\tilde{\epsilon}_q = |\epsilon_q|/\delta q$ and $\tilde{\epsilon}_p = |\epsilon_p|/\delta p$, the equation simplifies to a strictly scale-independent form:
\begin{equation}
\label{eq:DimensionlessErasureAction}
\text{Erasure Action} = \frac{A}{2} (\tilde{\epsilon}_q + \tilde{\epsilon}_p)
\end{equation}

Because the minimal-action path strictly limits the residual displacement to the bounds of the resolved grid, the relative errors satisfy $\tilde{\epsilon}_q \le 1$ and $\tilde{\epsilon}_p \le 1$. Evaluating the worst-case maximal displacement ($\tilde{\epsilon}_q = \tilde{\epsilon}_p = 1$) analytically recovers the thermodynamic bound defined in Eq.~\ref{eq:Hero}:
\begin{align}
\text{Erasure Action} &\le \frac{A}{2} (1 + 1) \nonumber \\
K E_L \tau &\le A
\end{align}
