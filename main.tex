...
\subsection{De Gosson: The Quantum Blob}
...
\subsection{Zurek: One Blob or Two?}
...
\subsection{Landauer: Information Erasure}

\begin{equation}
\label{eq:LandauerErasureEnergy}
E_L = k_B T \ln{2}
\end{equation}

\begin{equation}
\label{eq:LandauerErasure1Bit}
E_{L,K=1} = k_B T \ln{2}
\end{equation}

\begin{equation}
\label{eq:TotalLandauerErasureEnergy}
E_L = K k_B T \ln{2}
\end{equation}

\subsection{Margolus-Levitin: Erasure Action}

To express this thermodynamic limit as a phase-space volume, it is dimensionally necessary to introduce a characteristic erasure time, $\tau$:
...
Solving for $\tau$ exactly recovers the Margolus-Levitin limit for the minimum time to evolve to an orthogonal state ($\tau_\perp$):

\begin{equation}
\label{eq:MLErasureTime}
\tau = \frac{h}{4 E_L} = \tau_\perp
\end{equation}

This demonstrates that the Margolus-Levitin switching time naturally emerges as the limiting quantum case of a generalized thermodynamic erasure action.


\subsection{Lloyd: Erasure Action and Classical Action}
...
Lloyd demonstrated that the number of elementary logical operations a physical system can perform is fundamentally bounded by its total available action \cite{Lloyd2000}. Applying this limit to thermodynamics, the total action required to erase $K$ bits of information cannot exceed the classical action capacity, $A$, of the surrounding phase space:
...
\subsection{Planck: Quantum Capacity}

Planck demonstrated that a classical phase space of area $A$ has a capacity of $\Gamma = A/h$ quanta. Scaling the local quantum boundaries ($\delta q, \delta p$) by this macroscopic capacity, we construct the upper limit of the Erasure Action:
...
\subsection{Encoding and Erasure of Quantum States}
