...
\title{The Quantum of Action and Landauer's Principle}
...
Where the erasure paths from the least-action microstate $(q_\blacktriangledown, p_\blacktriangledown)$ to the blob centers $(q_b, p_b)$ are bounded by $|\epsilon_q| \le \delta q$ and $|\epsilon_p| \le \delta p$.
...
Standard phase-space formulations of quantum mechanics traditionally employ the Wigner quasi-probability distribution $W(q,p)$ \cite{Wigner1932}. However, because a finite system possesses a finite thermodynamic bit budget (N), the physically accessible phase space cannot be a continuous manifold; it must be a discretely resolved grid. We instead introduce the Landauer density $L(q,p)$ as a sum of discrete Iverson brackets \cite{Knuth1992}—mathematical switches that snap to exactly 1 when a coordinate is occupied and 0 otherwise—over the quantum blobs:
...
For any phase-space coordinate, $x \in \{q, p\}$, a partitioning function selects the least action microstate $x_\blacktriangledown$ from among infite possible microstates $\{x_\mu\}$ inside the squeezed boundary $\delta x$ centered on the blob coordinate $x_b$, yielding a binary sequence $s_x$:

\begin{equation}
\label{eq:PartitionFunction}
(x_\blacktriangledown, s_x) = \operatorname{PARTITION}(x_b, \delta x)
\end{equation}
...
\epsilon_x = \operatorname{ERASE}(s_x, x_\blacktriangledown, x_b), \quad \text{expends} \quad \Delta A_L = N_x E_L \tau
\end{equation}

The vector $\mathbf{\epsilon} = (\epsilon_q,\epsilon_p)$ is the distance from the least-action state $(q_\blacktriangledown, p_\blacktriangledown)$ to the blob center $(q_b, p_b)$. The observed classical coordinates $(q,p)$ are these displacements scaled by the geometric capacity $\frac{A}{\pi \hbar}$:
...
\Function{PARTITION}{$\widetilde{x_b}, \widetilde{\delta x}$}
    ...
    \State $\widetilde{x_\blacktriangledown} \gets \text{num} / \text{den}$
    \State $\text{right} \gets (1, 0)$
    \State $s_x \gets \text{``''}$
    \While{$|\widetilde{x_\blacktriangledown} - \widetilde{x_b}| > \widetilde{\delta x}$}
        \If{$\tilde{x}_\blacktriangledown < \tilde{x}_b$} 
        ...
        \State $\tilde{x}_\blacktriangledown \gets \text{num} / \text{den}$
    \EndWhile
    \State \Return $\tilde{x}_\blacktriangledown, s_x$
\EndFunction
\end{algorithmic}
...
\begin{algorithmic}[1]
\Function{ERASE}{$(\bar{s}_x)^R, \widetilde{x_\blacktriangledown}, \widetilde{x_b}$}
    ...
    \While{$N_x > 0$}
        ...
        \State $N_x \gets |(\bar{s}_x)^R|$
    \EndWhile
    \State $\widetilde{\epsilon_x} \gets \widetilde{x_\blacktriangledown} - \widetilde{x_b}$ \Comment{The normalized phase-space displacement}
