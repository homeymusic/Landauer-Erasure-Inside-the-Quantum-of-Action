...
System boundaries $\Delta q$ and $\Delta p$ determine the total action $A$, while the reciprocal relations $\delta q \propto 1 / \Delta p$ and $\delta p \propto 1 / \Delta q$ characterize the symplectic squeezing of the conjugate pair of action quanta: $A_{\delta q} = \frac{\pi}{4} \delta_q \Delta p$ and $A_{\delta p} = \frac{\pi}{4} \Delta q \delta p$. The values $\epsilon_q$ and $\epsilon_p$ are the erasure distances from the microstate $q_\mu, p_\mu$ with binary encoding $s_\mu$ to the minimal action state $(q^*,p^*)$ with the minimal binary encoding $\mathbf{s}^*$, where bit-length $|\mathbf{s}^*| \le |s_\mu|$; the physical scales satisfy $A_q + A_p = h$, $\epsilon_q \le \delta_q \le \Delta q$, and $\epsilon_p \le \delta_p \le \Delta p$.}
