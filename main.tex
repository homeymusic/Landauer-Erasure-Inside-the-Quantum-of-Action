...
In standard phase-space formulations, the density of states is modeled statistically using the continuous Wigner quasi-probability distribution $W(q,p)$. Here, the observed density is the deterministic result of the joint thermodynamic erasure distribution $\mathcal{E}(q,p)$.

When the environment erases a continuous microstate $(q_\mu, p_\mu)$ into the minimal-action state $(q^*, p^*)$, the physical erasure distances are defined as $\epsilon_q = q_\mu - q^*$ and $\epsilon_p = p_\mu - p^*$. The observed coordinates are directly proportional to this thermodynamic displacement:
...
Let $N$ be the total ensemble of observed microstates. The joint erasure distribution $\mathcal{E}(q,p)$ counts the exact number of microstates deterministically erased into each observed phase-space coordinate via the indicator function $\mathbb{I}$:

\begin{equation}
\label{eq:JointErasure}
\mathcal{E}(q,p) = \sum_{\mu=1}^{N} \mathbb{I}\left( \frac{A}{h} \epsilon_q = q \right) \mathbb{I}\left( \frac{A}{h} \epsilon_p = p \right)
\end{equation}

In standard phase-space mechanics, the observable marginal densities are obtained by integrating the Wigner function over the conjugate variable. In our deterministic framework, the discrete state counts $N(q)$ and $N(p)$ emerge naturally by summing the joint erasure distribution $\mathcal{E}(q,p)$ over the conjugate observables:

\begin{equation}
\label{eq:MarginalQ}
N(q) = \sum_{p} \mathcal{E}(q,p)
\end{equation}

\begin{equation}
\label{eq:MarginalP}
N(p) = \sum_{q} \mathcal{E}(q,p)
\end{equation}
