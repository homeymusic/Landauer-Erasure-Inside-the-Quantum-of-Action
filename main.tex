...
$$
s_q^* = \overline{s_q}^{\mathsf{R}}
\quad \text{and} \quad
s_p^* = \overline{s_p}^{\mathsf{R}}
$$
...
The binary sequence from the encoder $s_x$ is passed into the erasure function.
...
\Function{erase}{$s_x$}
    ...
    \For{$i \gets K \textbf{ down to } 1$}
        ...
        \Else 
            \State $s_x^* \gets s_x^* + \text{`1'}$
        \EndIf
        \State \text{Dissipate Landauer heat} $k_B T \ln{2}$ \text{over time} $\tau$
    \EndFor
    \State \Return $s_x^*$
\EndFunction
\end{algorithmic}
...
The binary sequence from the encoder $s_x$ is passed into the erasue function.
...
\Function{erase}{$s_x$}
    ...
    \For{$i \gets K \textbf{ down to } 1$}
        ...
        \Else 
            \State $s_x^* \gets s_x^* + \text{`1'}$
        \EndIf
        \State \text{Dissipate } $E_L \tau$ \text{Landauer action} 
    \EndFor
    \State \Return $s_x^*$
\EndFunction
\end{algorithmic}
